...
Deno.test("testObjects", async () => {
  ...
  if (ok) {
	...
	if (ok) {
		assertStrictEquals(ok, true, `expected ${key} -> ${sObj} to be updated, failed`);
        sObj = await ds.read(key);
	...
	assertStrictEquals(ok, true, `expected delete to return true for key ${key}`);
	keys = await ds.keys()
	assertStrictEquals(keys.length, 0, `expected zero keys after delete, got ${keys.length}`);
  }
